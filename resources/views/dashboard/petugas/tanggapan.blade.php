   ...
   <table class="table table-md table-zebra">
      ...
      <thead>
         <tr>
            ...
            <th class="text-[1.3em] text-center">Petugas</th>
      ...
      <tbody>
         ...
         <tr>
            <th class="text-center">{{ ($tanggapan->currentPage() - 1) * $tanggapan->perPage() + $loop->iteration }}
            </th>
            <td class="text-center">{{ $t->pengaduan->waktu_pengaduan }}</td>
            <td class="text-center">{{ $t->pengaduan->mahasiswa->nama_lengkap }}</td>
            <td class="text-center">{{ Str::limit(strip_tags($t->pengaduan->isi_laporan), 15) }}</td>
            <td class="text-center">{{ $t->pengaduan->status === 0 ? "PROSES" : "SELESAI" }}</td>
            <td class="text-center">{{ $t->petugas->nama_lengkap }}</td>
            <td class="text-center"><a href="/dashboard/pengaduan/{{ $t->pengaduan->id }}"
               ...
               <a href="/dashboard/tanggapan/{{ $t->pengaduan->id }}"
                  class="badge p-3 hover:bg-indigo-800 badge-primary"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                     height="16" fill="currentColor" class="bi bi-chat-left-text-fill" viewBox="0 0 16 16">
                     <path
               ...
               <form action="/dashboard/pengaduan/{{ $t->pengaduan->id }}" method="POST" class="inline">
                  @method("put")
                  @csrf
